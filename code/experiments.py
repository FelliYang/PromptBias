...
class Experiment():
    def __init__(self,full_exp) -> None:
        """
        ...
        # 该参数用于是否执行完整实验，在一些小实验中可以关闭，加快速度
        self.full_expriment = full_exp
        
        if self.full_expriment:
    ...
    def experiment_for_one_relation_new(self, relation:str, train_data:list, valid_data, test_data,ctrl_code=[1,1,1], weight_sampler=None):
        """
        提供可选的训练集、测试机配置
        """
        # 1. 处理好数据集
        ...
        # 允许训练数据集使用采样器
        if weight_sampler:
            train_samples_batches = DataLoader(train_data,batch_size=self.train_batchsize,sampler=weight_sampler,collate_fn=collate_fn)
        else:
        ...
        if ctrl_code[0] ==1:
            ...
            ckpt,(best_epoch,valid_loss) = self.train_model_new(os.path.join(self.output_dir,relation),model,train_samples_batches,valid_samples_batches,valid_sentences_batches,save_name=save_name)
            precision, test_loss,_ = self.evaluate_model_new(model, eval_samples_batches=test_samples_batches, eval_sentences_batches=test_sentences_batches,ckpt_path=ckpt)
    ...
    def frame_work(self, relation):
        """对于某个关系执行frame来得到精度"""
        # 当前仅支持P19，假设P19的uni_dataset 已经得到了
        # part1 - random test
        # 准备数据集需要使用到模型特定的MASK
        ...
        uni_data_dir = f"/home/jiao/code/prompt/OptiPrompt/data/uni/{relation}"
        train_data, _ = load_data(os.path.join(uni_data_dir,"train.json"), template, vocab_subset=self.bert_vocab, mask_token=model_MASK)
        valid_data, _ = load_data(os.path.join(uni_data_dir,"valid.json"), template, vocab_subset=self.bert_vocab, mask_token=model_MASK)
        lama_data_path = os.path.join(self.data_dir,relation,"test.jsonl")
        test_data, _ = load_data(lama_data_path, template, vocab_subset=lama_vocab_subset, mask_token=model_MASK)

        random_precision = self.random_test(relation, train_data, valid_data, test_data)
        

        # part2 - training and test
        lama_score, trained_model  = self.training(relation, train_data, valid_data, test_data)

        # part3 - bais test
        # 这里必须要确保传入的model是valid最好的那个，由于part2中已经做了这一步，因此这里的model已经满足要求了
        out = self.bais_test(relation, trained_model,test_data=valid_data)
        
        print("******frame work output*******")
        print(f"part1 precision {0}\n part2 score {lama_score}\n part3 output distribution:\n")
        print(out)

    def random_test(self, relation, train_data, valid_data, test_data,weight_sampler=None):
        precision, _ = self.training(relation, train_data,valid_data, test_data, weight_sampler=weight_sampler,random_init="all")
        return precision

    def training(self, relation, train_data, valid_data, test_data,weight_sampler=None, random_init="none"):
        model = Prober(args,random_init)
        prepare_for_dense_prompt(model)
        save_name=f"model-{random_init}-random_prompt-random-init.npy"
        save_path = "/home/jiao/code/prompt/OptiPrompt/outputs/experiment/frame_work/"+relation
        ...
        valid_samples_batches, valid_sentences_batches = batchify(valid_data, self.valid_batchsize)
        test_samples_batches, test_sentences_batches = batchify(test_data, self.valid_batchsize)
        ckpt,(best_epoch,valid_loss) = self.train_model_new(save_path,model,train_samples_batches,valid_samples_batches,valid_sentences_batches,save_name=save_name)
        precision, test_loss, _ = self.evaluate_model_new(model, eval_samples_batches=test_samples_batches, eval_sentences_batches=test_sentences_batches,ckpt_path=ckpt)
        return precision,model


    def bais_test(self, relation , model:Prober, test_data):
        # current only support P19
        eval_samples_batches, eval_sentences_batches = batchify(test_data, self.valid_batchsize)
        t1,t2, predict_result = self.evaluate_model_new(model, eval_samples_batches, eval_sentences_batches, common_vocab=False)
        obj_count = defaultdict(int)
        for d in predict_result[relation]:
            obj_count[d["topk"][0]["token"]]+=1
        
        uni_predict = sorted(obj_count.items(), key = lambda obj:obj[0])
        return uni_predict
        

    def type_of_obj(self, data:list):
    ...
    def count_obj(self, data:list):
        # 按照data中obj的类别，来统计出一个按照字母表排序的 (obj,num) list
        obj_count = defaultdict(int)
        for d in data:
    ...
    def repeat_check(self, data:list):
        # 检查数据中有多少个重复样本
        temp= set()
        ans = defaultdict(int)
        for d in data:
            t = (d["sub_label"],d["obj_label"])
            if t in temp:
                ans[d["obj_label"]] += 1
            temp_l = list(temp)
            temp_l.append(t)
    ...
    def experiment_trained_with_uni_N_times(self,N, ctrl_code=[1,1,1]):
        ...
        for relation in self.relations:
            ...
            collects = []
            for i in range(N):
                collect = self.experiment_for_one_relation_new(relation,train_data,valid_data,test_data,ctrl_code=ctrl_code,weight_sampler=weight)
    ...
    def train_model_new(self, save_dir:str, model:Prober, train_samples_batches:DataLoader, valid_samples_batches, valid_sentences_batches, save_name="model-none-random_prompt-random-init.npy"):
    ...
    def evaluate_model_new(self, model:Prober, eval_samples_batches, eval_sentences_batches,ckpt_path=None, common_vocab=True):
        # FIXME 暂时只支持random初始化的prompt
        # 使用传入的model直接对数据集评估，可以通过ckpt参数来重新初始化模型的prompt参数
        # 请确保传入的model已经添加过随机初始化过的token了
        original_vocab_size = len(list(model.tokenizer.get_vocab())) - MAX_NUM_VECTORS
        if ckpt_path:
        ...
        return precision,loss,predict_result
    ...
    def generate_uni_dataset(self, relation):
        """
        生成uni数据集, 该函数暂时只支持P19
        """
        # 1. 读入list格式的数据
        template = init_template(args,model=None)
        uni_data_dir = "/home/jiao/code/prompt/OptiPrompt/data/wiki_uni"
        uni_data_path = os.path.join(uni_data_dir,relation)
        uni_data = load_json(uni_data_path)
        res = self.repeat_check(uni_data)
        print(res)
        # 2. 划分成train和valid两个list，使用分层划分 暂时不考虑重复
        type_list = self.type_of_obj(uni_data)
        train, valid,_,_ = train_test_split(uni_data,type_list,test_size=0.2,random_state=1,stratify=type_list)
        # 3. 分别保存成两个json文件
        save_path = "/home/jiao/code/prompt/OptiPrompt/data/uni"+ "/"+relation
    ...
    def get_prompt_bias(self):
        """
        构造[mask][template][mask]下，最后mask的概率分布，以此反映出来模型的bias
        暂时只支持place of birth
        使用huggingface 直接实现，脱离原有框架
        """
        input = "The native language of [MASK] is [MASK] ."
        tokeizer = AutoTokenizer.from_pretrained("bert-base-cased")
        model = AutoModelForMaskedLM.from_pretrained("bert-base-cased")
        
        model_input = tokeizer(input,return_tensors="pt")
        
        output = model(**model_input)
        distribution = output[0][0][-3]
        probs = torch.softmax(distribution,0).tolist()
        
        probs = [(prob, probs.index(prob)) for prob in probs ]
        # sort
        ...
        total = 0
        for i in range(100):
            prob = sorted_probs[i][0]
            total += prob
            word = tokeizer.decode(sorted_probs[i][1])
            print(word, prob)
        
        print(f"前{N}个概率占{total}")
    ...
    def train_model(self, relation:list, random_init="none", prompt_init="random", random_weight_index=0):
        ...
        if self.obj_resample:
            weight_sampler = WeightedRandomSampler(weight, len(weight))
            train_samples_batches = DataLoader(train_data, batch_size=self.train_batchsize,sampler=weight_sampler,collate_fn=collate_fn)
        else:
...
exp = Experiment(full_exp=False)

# exp.generate_uni_dataset("P19")
# exp.frame_work("P19")
exp.get_prompt_bias()

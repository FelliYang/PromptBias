...
do
    ...
    bash $run_sh $model_type $model_name $inter_vocab typed_querying LAMA $((2*i+0))
    bash $run_sh $model_type $model_name $inter_vocab typed_querying LPAQA $((2*i+0))
    bash $run_sh $model_type $model_name $inter_vocab typed_querying AutoPrompt $((2*i+1))
    bash $run_sh $model_type $model_name $inter_vocab typed_querying optiprompt $((2*i+1))

...
model_types=("llama")
model_names=("/mnt/data/users/xuziyang/huggingface_model/shares/llama2-convert-7b-hf")
...
do
    ...
    # bash $run_sh $model_type $model_name $inter_vocab typed_querying LAMA $((1*i+3)) True False 0 $filter_biased_token_nums $ABLATION_NO_NORMALIZATION $ABLATION_NO_RESCALE
    bash $run_sh $model_type $model_name $inter_vocab typed_querying LPAQA $((1*i+3)) True False 0 $filter_biased_token_nums $ABLATION_NO_NORMALIZATION $ABLATION_NO_RESCALE
